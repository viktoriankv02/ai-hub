...
export function createAIJobExecutor(options: AIJobRuntimeOptions = {}): AIJobExecutor {
  ...
  if (mode !== "openai-compatible") throw new Error(`unsupported AI_JOB_EXECUTOR: ${mode}`);
  ...
  const provider = new OpenAICompatibleProvider({
    ...
    maxRetries: options.maxRetries ?? numberFromEnv(process.env.AI_PROVIDER_MAX_RETRIES),
    retryBaseDelayMs: options.retryBaseDelayMs ?? numberFromEnv(process.env.AI_PROVIDER_RETRY_BASE_DELAY_MS),
    retryMaxDelayMs: options.retryMaxDelayMs ?? numberFromEnv(process.env.AI_PROVIDER_RETRY_MAX_DELAY_MS),
  });
