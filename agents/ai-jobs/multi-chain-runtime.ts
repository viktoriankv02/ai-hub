...
function buildRuntime(target: MultiChainEVMTarget): EVMOnchainRuntime {
  ...
  // The existing single-chain runtime reads AI_JOB_COMPLETION_STORE from the
  // environment. Isolate that process-global setting while constructing each
  // target so a multi-chain process cannot accidentally share completion
  // publication state between networks.
  const previous = process.env.AI_JOB_COMPLETION_STORE;
  if (target.completionStorePath) {
    process.env.AI_JOB_COMPLETION_STORE = resolve(target.completionStorePath);
  } else {
    delete process.env.AI_JOB_COMPLETION_STORE;
  }

  try {
    return createEVMOnchainRuntime(options);
  } finally {
    if (previous === undefined) delete process.env.AI_JOB_COMPLETION_STORE;
    else process.env.AI_JOB_COMPLETION_STORE = previous;
  }
}
