...
export interface MultiChainHttpApiOptions {
  token?: string;
  maxBodyBytes?: number;
}
 ...
 *
 * targetId can be supplied as a query parameter. Otherwise the job's
 * chainTargetId is used, then the runtime's default target is selected.
 */
  ...
  async handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (match && req.method === "POST") {
        ...
        if (action === "provision") {
          return this.write(res, 200, { result: await this.multiChain.provision(job, targetId) });
        }
        if (action === "complete") {
          return this.write(res, 200, { result: await this.multiChain.complete(job, targetId) });
        }
