...
export function createEVMOnchainRuntime(options: EVMOnchainRuntimeOptions): EVMOnchainRuntime {
  ...
  const completionStore = options.completionStorePath
    ? new JsonCompletionPublicationStore(resolve(options.completionStorePath))
    : undefined;
  ...
  const resolveAgentId = async (agentId: string): Promise<bigint> => {
    ...
    throw new Error(
      `cannot resolve off-chain agentId '${agentId}' to a positive on-chain agent id`,
    );
  };
