...
export class ExecutionGate {
  ...
  evaluate(request: ExecutionGateRequest): ExecutionGateDecision {
    if (request.mode === "dry-run") {
      return { allowed: true, requiresConfirmation: false, reason: "dry-run does not perform external side effects" };
    }
    if (riskRank(request.risk) > riskRank(this.policy.maxRisk)) {
      return { allowed: false, requiresConfirmation: true, reason: `risk ${request.risk} exceeds policy limit ${this.policy.maxRisk}` };
    }
    if (request.requiresWallet && !request.walletConnected) {
      return { allowed: false, requiresConfirmation: false, reason: "wallet connection is required" };
    }
    if (request.requiresGas && !request.gasAvailable) {
      return { allowed: false, requiresConfirmation: false, reason: "gas availability is required" };
    }
    ...
    if (approvalRequired && !request.approved) {
      return { allowed: false, requiresConfirmation: true, reason: "explicit user approval is required" };
    }
