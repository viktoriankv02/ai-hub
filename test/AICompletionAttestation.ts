...
describe("AI completion attestation", function () {
  ...
  it("rejects malformed signatures without throwing from the verifier", async function () {
    const signer = Wallet.createRandom();
    const attestation = await createCompletionAttestation(completedJob(), signer);

    expect(verifyCompletionAttestation({ ...attestation, signature: "0x1234" })).to.equal(false);
    expect(() => assertValidCompletionAttestation({ ...attestation, signature: "0x1234" })).to.throw(
      "invalid completion attestation signature",
    );
  });
  ...
  it("derives a stable completion id and result bytes32 hash", async function () {
    const signer = Wallet.createRandom();
    const payload = payloadFromJob(completedJob());
    const address = await signer.getAddress();

    expect(completionIdFromPayload(payload, address)).to.equal(
      completionIdFromPayload(payload, address.toLowerCase()),
    );
    expect(completionIdFromPayload(payload, address)).to.match(/^0x[0-9a-fA-F]{64}$/);
    expect(resultHashBytes32(payload.resultHash)).to.match(/^0x[0-9a-fA-F]{64}$/);
    expect(resultHashBytes32(payload.resultHash)).to.equal(resultHashBytes32(payload.resultHash));
  });
});
