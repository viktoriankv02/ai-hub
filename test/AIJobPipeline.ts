...
describe("AI agent job pipeline", function () {
  ...
  it("exposes a completion deadline when timeout protection is enabled", async function () {
    ...
    await (await engine.setJobRiskLimits(0, 60, 0)).wait();
    await (await engine.connect(developer).createJob(1, ethers.id("TASK_DEADLINE"), ethers.parseEther("1"))).wait();
    const job = await engine.jobs(1);
    expect(await engine.jobExecutionDeadline(1)).to.equal(job.createdAt + 60n);
    await (await engine.connect(owner).assignJob(1)).wait();
    await ethers.provider.send("evm_increaseTime", [61]);
    await ethers.provider.send("evm_mine", []);
    expect(await engine.jobExpired(1)).to.equal(true);
    await expect(engine.connect(system.reporter).completeJob(1, ethers.id("TOO_LATE"))).to.be.revertedWithCustomError(engine, "JobCompletionExpired");
    const before = await system.token.balanceOf(await developer.getAddress());
    await engine.connect(system.reporter).cancelExpiredJob(1);
    const after = await system.token.balanceOf(await developer.getAddress());
    expect(after - before).to.equal(ethers.parseEther("1"));
    expect(await engine.openJobsByCreator(await developer.getAddress())).to.equal(0n);
    expect((await engine.jobs(1)).reward).to.equal(0n);
  });
});
