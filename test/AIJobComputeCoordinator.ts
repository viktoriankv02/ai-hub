...
describe("AIJobComputeCoordinator", function () {
  async function deploySystem() {
    ...
    const engine = await ethers.deployContract("AIAgentEngine", [await owner.getAddress(), await runtime.getAddress(), await token.getAddress()]);
    await engine.waitForDeployment();
    const nodes = await ethers.deployContract("AIComputeNodeRegistry", [await owner.getAddress(), await token.getAddress(), 1n]);
    await nodes.waitForDeployment();
    const coordinator = await ethers.deployContract("AIJobComputeCoordinator", [await owner.getAddress(), await engine.getAddress(), await nodes.getAddress()]);
    ...
    await (await nodes.connect(developer).registerNode("https://compute.example", "A100", 80, 32, 128, "eu-west", 1n)).wait();

    return { owner, developer, controller, token, runtime, engine, nodes, coordinator };
  }

  it("binds an assigned job to an available compute node", async function () {
  ...
  it("fails an execution without marking the AI job as completed", async function () {
    const system = await deploySystem();
    const { controller, coordinator, nodes } = system;
    await (await coordinator.connect(controller).bindAndStart(1, 1)).wait();
