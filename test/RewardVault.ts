...
describe("RewardVault", function () {
  ...
  it("rejects unauthorized managers", async function () {
    ...
    const vault = await ethers.deployContract("RewardVault", [owner.address]);
    await expect(vault.connect(user).claimNative(ethers.id("UNAUTHORIZED"), user.address, 1n)).to.be.revertedWith("Vault: unauthorized manager");
  });
  ...
  it("enforces native per-claim and daily treasury limits", async function () {
    ...
    await expect(vault.connect(manager).claimNative(ethers.id("LIMIT_BIG"), user.address, ethers.parseEther("0.21"))).to.be.revertedWith("Vault: native claim exceeds max");
    await vault.connect(manager).claimNative(ethers.id("LIMIT_1"), user.address, ethers.parseEther("0.2"));
    await expect(vault.connect(manager).claimNative(ethers.id("LIMIT_2"), user.address, ethers.parseEther("0.11"))).to.be.revertedWith("Vault: native daily budget exceeded");
  ...
  it("enforces ERC20 per-claim and daily token budgets", async function () {
    ...
    await expect(vault.connect(manager).claimERC20(ethers.id("ERC20_BIG"), token.target, user.address, 201n)).to.be.revertedWith("Vault: ERC20 claim exceeds max");
    await vault.connect(manager).claimERC20(ethers.id("ERC20_LIMIT_1"), token.target, user.address, 200n);
    await expect(vault.connect(manager).claimERC20(ethers.id("ERC20_LIMIT_2"), token.target, user.address, 101n)).to.be.revertedWith("Vault: ERC20 daily budget exceeded");
