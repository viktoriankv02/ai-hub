...
contract RewardVault is Ownable, Pausable, ReentrancyGuard {
    ...
    function setRiskLimits(uint256 newMaxNativeClaim, uint256 newMaxERC20Claim, uint256 newDailyNativeBudget) external onlyOwner {
    ...
    function nativeRiskState() external view returns (uint256 spent, uint256 budget, uint256 resetAt) {
        if (dailyNativeSpentAt == 0 || block.timestamp >= dailyNativeSpentAt + DAY) return (0, dailyNativeBudget, block.timestamp + DAY);
    ...
    function erc20RiskState(address token) external view returns (uint256 spent, uint256 budget, uint256 resetAt) {
        ...
        if (startedAt == 0 || block.timestamp >= startedAt + DAY) return (0, erc20DailyBudget[token], block.timestamp + DAY);
    ...
    function _checkNativeLimits(uint256 amount) internal view {
        ...
        if (dailyNativeBudget > 0) require(_currentNativeSpent() + amount <= dailyNativeBudget, "Vault: native daily budget exceeded");
    }
    ...
    function _checkERC20Limits(address token, uint256 amount) internal view {
        ...
        if (budget > 0) require(_currentERC20Spent(token) + amount <= budget, "Vault: ERC20 daily budget exceeded");
    }
    ...
    function _recordERC20Spend(address token, uint256 amount) internal {
        ...
        } else erc20DailySpent[token] += amount;
    }
