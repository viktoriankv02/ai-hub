...
/// @dev Node lifecycle mutations that affect jobs are controller-authorized;
///      node owners retain control over metadata, heartbeat and disabling.
contract AIComputeNodeRegistry is Ownable {
    ...
    function ownerNodes(address ownerAddress) external view returns (uint256[] memory) {
        return _ownerNodes[ownerAddress];
    }
