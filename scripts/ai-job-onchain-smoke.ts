...
const rpcUrl = process.env.AI_JOB_RPC_URL ?? process.env.BASE_SEPOLIA_RPC_URL ?? process.env.BASE_RPC_URL ?? "";
...
if (!rpcUrl || !privateKey || !engineAddress || !rewardTokenAddress || !reporterAddress) {
  throw new Error("AI_JOB_RPC_URL/BASE_SEPOLIA_RPC_URL/BASE_RPC_URL, AI_JOB_PRIVATE_KEY/DEPLOYER_PRIVATE_KEY, AI_AGENT_ENGINE_ADDRESS, AI_REWARD_TOKEN_ADDRESS and AI_COMPLETION_REPORTER_ADDRESS are required");
}
...
const signer = await onchain.signer.getAddress();
