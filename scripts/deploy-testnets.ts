...
for (const target of targets) {
  ...
  if (!config) {
    throw new Error(`Unsupported AI Hub network: ${target}`);
  }

  if (!config.testnet) {
    throw new Error(`Refusing non-testnet deployment: ${config.name}`);
  }
...
if (!process.env.AI_HUB_ADMIN_ADDRESS) {
  throw new Error("Missing AI_HUB_ADMIN_ADDRESS");
}
  ...
  for (const step of steps) {
    ...
    const args =
      process.platform === "win32"
        ? ["/d", "/s", "/c", `npx hardhat run ${step} --network ${target}`]
